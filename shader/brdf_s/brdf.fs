...
vec3 GetColorAfterExposure(vec3 color,float exposure_par)
{
    vec3 res = vec3(1.0f) - exp(-color*exposure_par);
    return res;
}
void main()
{
    ...
    for(int i=0;i<dir_light_num;i++)
    {
        ...
        res_color += (albedo_/PI +specular)*radiance*NdotL;
    }
    ...
    res_color = GetColorAfterExposure(res_color,1.0f);
